...
static int sun4i_i2s_set_clkdiv(struct snd_soc_dai *cpu_dai,
				int div_id, int div)
{
	u32 reg;
	switch (div_id) {
...
{
	u32 reg_val;
	printk(KERN_INFO, "[IIS]Entered %s\n", __func__);
	...
	printk(KERN_INFO, "[IIS]SPECIAL CLK 0x01c20068 = %#x, line= %d\n", *(volatile int*)0xF1C20068, __LINE__);
	printk(KERN_INFO, "[IIS]SPECIAL CLK 0x01c200B8 = %#x, line = %d\n", *(volatile int*)0xF1C200B8, __LINE__);
...
static int __devinit sun4i_i2s_dev_probe(struct platform_device *pdev)
{
	...
	if (-1 == clk_enable(i2s_apbclk)) {
		printk(KERN_WARN, "i2s_apbclk failed! line = %d\n", __LINE__);
		goto out;
	}
	...
	if (clk_set_parent(i2s_moduleclk, i2s_pll2clk)) {
		printk(KERN_WARN, "try to set parent of i2s_moduleclk to i2s_pll2ck failed! line = %d\n", __LINE__);
		goto out1;
	}

	if (clk_set_rate(i2s_moduleclk, 24576000/8)) {
		printk(KERN_WARN, "set i2s_moduleclk clock freq to 24576000 failed! line = %d\n", __LINE__);
		goto out1;
	}

	if (-1 == clk_enable(i2s_moduleclk)) {
		printk(KERN_WARN, "open i2s_moduleclk failed! line = %d\n", __LINE__);
		goto out1;
	}
...
static int __init sun4i_i2s_init(void)
{
	...
	if (ret)
		printk(KERN_WARN, "[I2S]sun4i_i2s_init fetch i2s using configuration failed\n");


	if (i2s_used) {
		i2s_handle = gpio_request_ex("i2s_para", NULL);

		err = platform_device_register(&sun4i_i2s_device)
		if (err < 0)
			return err;

	} else {
	printk(KERN_WARN, "[I2S]sun4i-i2s cannot find any using configuration for controllers, return directly!\n");
	return 0;
	}
	return 0;
}
